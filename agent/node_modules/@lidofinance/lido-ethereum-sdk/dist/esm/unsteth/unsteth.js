...
import { getContract, zeroAddress, encodeFunctionData, } from 'viem';
...
let LidoSDKUnstETH = (() => {
    ...
    return _a = class LidoSDKUnstETH extends _classSuper {
            ...
            async setSingleTokenApproval(props) {
                const { account, callback, to = zeroAddress, id, ...rest } = await this.parseProps(props);
                const args = [to, id];
                const contract = await this.getContract();
                return this.core.performTransaction({
                    ...rest,
            ...
            async getContractMetadata() {
                if (this.core.rpcProvider.multicall) {
                    const address = await this.contractAddress();
                    const common = { abi: unstethAbi, address };
        ...
        (() => {
            ...
            _getContract_decorators = [Logger('Contracts:'), Cache(30 * 60 * 1000, ['core.chain.id', 'contractAddressWstETH'])];
            _getNFTsByAccount_decorators = [Logger('Balances:'), ErrorHandler()];
            _getAccountByNFT_decorators = [Logger('Balances:'), ErrorHandler()];
            _transfer_decorators = [Logger('Call:'), ErrorHandler()];
            _transferSimulateTx_decorators = [Logger('Utils:'), ErrorHandler()];
            _transferPopulateTx_decorators = [Logger('Utils:'), ErrorHandler()];
            _getSingleTokenApproval_decorators = [Logger('Views:'), ErrorHandler()];
            _setSingleTokenApproval_decorators = [Logger('Call:'), ErrorHandler()];
            _setSingleTokenApprovalPopulateTx_decorators = [Logger('Utils:'), ErrorHandler()];
            _setSingleTokenApprovalSimulateTx_decorators = [Logger('Utils:'), ErrorHandler()];
            _areAllTokensApproved_decorators = [Logger('Views:'), ErrorHandler()];
            _setAllTokensApproval_decorators = [Logger('Call:'), ErrorHandler()];
            _setAllTokensApprovalPopulateTx_decorators = [Logger('Call:'), ErrorHandler()];
            _setAllTokensApprovalSimulateTx_decorators = [Logger('Call:'), ErrorHandler()];
            _getContractMetadata_decorators = [Logger('Views:'), ErrorHandler(), Cache(30 * 60 * 1000, ['core.chain.id'])];
            _getTokenMetadataURI_decorators = [Logger('Views:'), ErrorHandler()];
