...
var buyCredit = async (recall, _context, params) => {
  try {
    ...
    const { meta } = await recall.creditManager().buy(parseEther(params.amount), to);
...
var createBucketParameters = (_context = {}) => z.object({
...
var getOrCreateBucketParameters = (_context = {}) => z.object({
  bucketAlias: z.string().describe("The alias of the bucket to retrieve or create"),
  metadata: z.record(
    z.string().min(1).describe("The key of the metadata"),
    z.string().min(1).describe("The value of the metadata")
  ).optional().describe("The metadata to store with the bucket")
});
var addObjectParameters = (_context = {}) => z.object({
  ...
  overwrite: z.boolean().optional().describe("Whether to overwrite existing data at that key")
});
var getObjectParameters = (_context = {}) => z.object({
  bucket: addressSchema.describe("The address of the bucket"),
  key: z.string().min(1).describe("The key under which the object is stored")
});
var queryObjectsParameters = (_context = {}) => z.object({
  bucket: addressSchema.describe("The address of the bucket"),
  prefix: z.string().min(1).optional().describe("The prefix of the objects to query"),
  delimiter: z.string().min(1).optional().describe("The delimiter of the objects to query"),
  startKey: z.string().min(1).optional().describe("The starting key of the objects to query"),
  limit: z.number().min(1).optional().describe("The maximum number of objects to query")
});
...
var RecallAPI = class {
  ...
  constructor(privateKey, context, serializer = jsonStringify) {
    const chain = context?.network !== void 0 && checkChainName(context.network) ? getChain(context.network) : testnet;
  ...
  async run(method, arg) {
    const tool2 = this._tools.find((t) => t.method === method);
    if (tool2 !== void 0) {
      const output = this._serialize(
        await tool2.execute(this._recall, this._context, arg)
      );
      return output;
    } else {
...
// src/ai-sdk/tool.ts
import { tool } from "ai";
function RecallTool(recallAPI, method, description, schema) {
  return tool({
    description,
    parameters: schema,
    execute: (arg) => {
...
var RecallAgentToolkit = class {
  ...
  constructor({
    ...
    this.tools = {};
    const filteredTools = this._recall.getTools().filter((tool2) => isToolAllowed(tool2, configuration));
    filteredTools.forEach((tool2) => {
      this.tools[tool2.method] = RecallTool(
        this._recall,
        tool2.method,
        tool2.description,
        tool2.parameters
